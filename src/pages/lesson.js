...
import { findCourse, findTopic, findSubtopic, isGroup, leafSubtopics, loadLesson, loadQuiz } from '../catalog.js';
...
export async function renderLesson(root, catalog, params, query = {}) {
  ...
  try {
    const [lesson, quiz] = await Promise.all([loadLesson(path), loadQuiz(path)]);
